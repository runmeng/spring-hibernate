...
public class CategoryController {
    ...
    public String updateCategory(@Valid Category category , BindingResult result, RedirectAttributes rd) {
        if (result.hasErrors()) {
            rd.addFlashAttribute("org.springframework.validation.BindingResult.category",result);
        ...
        rd.addFlashAttribute("flash", new FlashMessage("Category updated successfully!", FlashMessage.Status.SUCCESS));
    ...
    public String addCategory(@Valid Category category , BindingResult result, RedirectAttributes rd) {
        if (result.hasErrors()) {
            rd.addFlashAttribute("org.springframework.validation.BindingResult.category",result);
            rd.addFlashAttribute("category", category);
        ...
        // TODO: Add category if valid data was received
        categoryService.save(category);
        rd.addFlashAttribute("flash", new FlashMessage("Category added successfully!", FlashMessage.Status.SUCCESS));
        // TODO: Redirect browser to /categories
    ...
    public String deleteCategory(@PathVariable Long categoryId,RedirectAttributes redirectAttributes) {
        ...
        if (category.getGifs().size()>0){
            redirectAttributes.addFlashAttribute("Only Empty category can be deleted", FlashMessage.Status.FAILURE);
